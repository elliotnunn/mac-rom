/*
	...
	Change History (most recent first):
		...
		<10>	 1/11/92	KST		Adding code to support SWM Background application.
		 ...
		 <8>	  1/9/92	KST		In "UseInputWindow", If idocID ==  0, it affects only one (ее
									current ее) application, not the whole system.
...
/*
Note: ее	The "TSMVarsPtr->tsmUseOldJInputMethodP" flag is meaningful only when the current
	  ее	script is Japanese. This means only test/change the flag in Japanese script !!
	  
	  ее	Thru out this and other files, I used 'Akiko' to refer to a TSM styled new Input Method.
*/
 ...
 *	ее Note ее	If script/language is not changed, this call is a NOOP.				*
 *																					*
 *	Modification History:															*
 ...
 *	ее SWM should never call this routine. Otherwise ... ее					*
 *																			*
 * Modification history:													*
 ...
 *																		*
 *	ее Note: If ts == kCurrentProcess, then we assume the window is		*
								...
								WindowPtr			*window )
{
	...
	if ( err==noErr) {									/*  to create OR locate a floating layer	*/
		...
		// ее need to fix 4 the next release ......
