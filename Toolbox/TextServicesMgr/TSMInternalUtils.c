/*
	...
	Copyright:	й 1992 by Apple Computer, Inc., all rights reserved.
 ...
 *
 *
 *	utActivateIM4Document:			<< ее already open, just activate >>
 ...
 *	utOpenIM4Document:				<< ее open an IM only >>
 ...
 *	utOpenActivateDefaultIM:		<< ее open or activate >>
 ...
 *	utActivateThisDocsInputMethod:	<< ее synchronize IM used by the document >>
 ...
 * ее	Warning:1. Assume theScript is one of the CCJK scripts.						*
 ...
 * ее Warning:	1. Assume no input method of this script opened for this document !!		*
 ...
 * ее What to do if we cannot find or cannot open the default input method ??????????	*
 *																						*
 * Modification history:																*
 ...
 * ее This document could be active and the input method's is not in	*
...
OSErr	utActivateThisDocsInputMethod(TSMDocumentID	idocID)
{
	...
	return err;
									...
									/* ее not complete yet (is this necessary?) */
 ...
 * ее Note: Assume "utScriptIsNotCCJK" has been called before using this function.			*
 *																							*
 * Modification history:																	*
 ...
 *					  user starts typing.										*
 *			ее Note that we don't deactivate TSM aware app's document.			*
...
void utFixDocumentAndCloseSWMWindow(ScriptCode	deactivateScript)
{
	...
	if ( currentTSMActiveDocID ) {					/* if currently has active document		<01Feb92 #21> */
		...
		if (currentActiveDocPtr->iDocPSNRecord == TSMVarsPtr->tsmSWMsPSNRecord) {		/* doc belongs to SWM	<#28> */
			/* ее must call DeactivateTSMDocument and utCloseSWMWindow before we change tsmCurrentSLRec *
			 * ее because DeactivateTSMDocument relies on current script and current IM are in sync !	*
			 * ее also, I'll have to call DeactivateTSMDocument here, because when SWM receive the AE,	*
			 * ее it is after xSetTextServiceLanguage routine already been executed ...					*/
